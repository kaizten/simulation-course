...
class MobilityModel:
    def __init__(self, env, num_vehicles):
        self.env = env
        self.vehicle_resource = simpy.PreemptiveResource(env, num_vehicles)
...
def person_trip(env, person_id, mobility_model, priority):
    ...
    with mobility_model.vehicle_resource.request(priority=priority, preempt=True) as request:
        try:
            yield request
            ...
            print(f"{env.now:.2f}: {person_name} was preempted by {by}.")
